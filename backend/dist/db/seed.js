...
import { createUser, createProduct, createSubmission, } from './queries.js';
...
async function seed() {
    try {
        ...
        const product1 = await createProduct('5010477010002', 'Organic Whole Grain Bread', 'Nature\'s Best', 'Bakery', ingredients.slice(0, 3), ['organic', 'vegan'], { calories: 150, protein: '8g', carbs: '30g' }, user1.id);
        const product2 = await createProduct('5010477010003', 'Almond Butter', 'Health Foods', 'Spreads', [ingredients[3]], ['organic', 'gluten-free'], { calories: 190, protein: '7g', fat: '17g' }, user2.id);
        // Create sample submissions (pending review)
        console.log('Creating sample submissions...');
        const submission1 = await createSubmission(user1.id, '5010477010004', 'Plant-Based Yogurt', 'Green Earth', 'Dairy Alternatives', [
            { id: 6, name: 'Coconut Milk', allergen: true },
            { id: 7, name: 'Probiotic Cultures', allergen: false },
        ], ['vegan', 'dairy-free'], { calories: 120, protein: '4g', carbs: '15g' }, 'https://example.com/product', 'Contains coconut milk alternative to dairy', 'pending_review');
