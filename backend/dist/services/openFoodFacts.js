...
function normalizeProduct(product) {
    ...
    if (ingredients.length === 0 && product.ingredients_text) {
        const ingredientNames = product.ingredients_text
            ...
            .map((s) => s.trim())
            .filter((s) => s.length > 0);
        ingredients.push(...ingredientNames.map((name, idx) => ({
            id: idx + 1,
            name,
            allergen: false,
        })));
    ...
    if (product.labels) {
        ...
        if (labels.includes('gluten-free'))
            certifications.push('gluten-free');
        if (labels.includes('fair-trade') || labels.includes('fairtrade'))
            certifications.push('fair-trade');
        if (labels.includes('kosher'))
            certifications.push('kosher');
        if (labels.includes('halal'))
            certifications.push('halal');
    }
    // Parse allergens
    const allergens = [];
...
//# sourceMappingURL=openFoodFacts.js.map