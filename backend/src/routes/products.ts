...
// Protected endpoints
router.post('/submit', authMiddleware, submitRateLimiter, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
router.get('/my-submissions', authMiddleware, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const userId = req.userId!;
